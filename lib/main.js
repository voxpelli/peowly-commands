...
export async function peowlyCommands (commands, options, meta) {
  ...
  if (showHelpOnNoCommand) {
    // ...but at the end of the day we're here to be helpful
    cli.showHelp(0);
  } else {
    throw new PeowlyCommandOmittedError(cli.showHelp);
  }
}
...
export class PeowlyCommandOmittedError extends Error {
  /** @override */
  name = 'PeowlyCommandOmittedError';

  /**
   * @param {(exitCode?: number) => never} showHelp
   */
  constructor (showHelp) {
    super('Command omitted');
